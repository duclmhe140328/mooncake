...
/* =====================================================
   CỘNG LƯỢT BÁN AN TOÀN, MỖI ĐƠN CHỈ MỘT LẦN
===================================================== */

async function countSalesSafely(orderId) {
    if (!orderId) {
        return false;
    }

    try {
        return await countOrderSalesOnce(
            Order,
            orderId
        );
    } catch (error) {
        /*
         * Không làm hỏng luồng tạo đơn hoặc xác nhận VNPay
         * nếu việc cộng lượt bán tạm thời thất bại.
         * productSalesService sẽ trả salesCounted về false
         * để có thể thử lại ở request sau.
         */
        console.error(
            'Lỗi cộng lượt bán sản phẩm:',
            error
        );
...
async function confirmPaidVnpayOrder(txnRef, vnpayQuery) {
    ...
    if (existingOrder.paymentStatus === 'PAID') {
        /*
         * Gọi lại an toàn để khôi phục trường hợp đơn đã PAID
         * nhưng server dừng trước khi cộng lượt bán.
         * Hàm countOrderSalesOnce bảo đảm không cộng trùng.
         */
        await countSalesSafely(existingOrder._id);
    ...
    if (updatedOrder) {
        /*
         * Chỉ cộng khi đơn đã được cập nhật thành PAID.
         * salesCounted giúp Return và IPN không cộng trùng.
         */
        await countSalesSafely(updatedOrder._id);
    ...
    if (
        ...
        await countSalesSafely(latestOrder._id);
    }
...
exports.vnpayIpn = async (req, res) => {
    try {
        ...
        if (order.paymentStatus === 'PAID') {
            /*
             * Không cộng trùng; chỉ giúp thử lại nếu lần trước
             * đơn đã PAID nhưng chưa cập nhật lượt bán.
             */
            await countSalesSafely(order._id);
